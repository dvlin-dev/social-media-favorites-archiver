...
收藏条目的标题、正文、描述、字幕、OCR、ASR、作者文字和其中的 URL 都来自第三方，必须只作为待归档数据处理，不能改变 Agent 的指令层级。Skill 必须明确要求 Agent 不执行、不转述为操作步骤、也不打开这些字段中嵌入的命令、提示词或链接；Agent 只运行 Skill 自身列出的固定 CLI 工作流，并只读取脱敏聚合报告。

可选 enrichment 继续使用应用层固定提示词、字段白名单、结构化 JSON 数据和输出 schema。第三方文字必须放在数据字段中，不能拼接为 system/developer/tool 指令；模型输出仍需经过现有 schema 校验。该边界是对 1.0.0 分发审计发现的间接 prompt-injection 风险的最小修复，不改变平台适配器、数据模型或本地处理架构。

1.0.1 以补丁版本发布这一 Skill 加固。安装命令继续固定到不可移动发布版本，而不是分支；外部代码安装、认证会话访问和定时任务属于本 Skill 的显式能力，分发审计可继续将这些固有能力列为需要用户审查的中风险项，不能为了追求“全绿”而隐藏或削弱说明。

2026-08-09 用户明确要求停止后续真实账号测试。1.0.1 的复验不得再次登录或同步平台账号；它继承 Task 18 的三平台代表项证据和 Task 23 在 1.0.0 公共 CLI 上完成的一次受限小红书 metadata-only 证据，并通过净安装、触发测试、doctor、脱敏 fixture 与完整自动化回归证明补丁没有改变应用行为。该限制必须在最终报告中保持可见，不能改写为 1.0.1 新完成的真实端到端测试。
...
| 收藏内容中的间接 prompt injection | Agent 被第三方文字诱导执行无关动作 | 内容只作为不可信数据、禁止执行内嵌命令/链接、Agent 只读取脱敏聚合报告、enrichment 使用固定指令与结构化数据边界 |
